...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_capacity_metrics.ephemeral_ports
        and context.nautobot_capacity_metrics.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_capacity_metrics.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_capacity_metrics.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_capacity_metrics.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_capacity_metrics.python_ver
    dockerfile_path = os.path.join(context.nautobot_capacity_metrics.compose_dir, "Dockerfile")
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_capacity_metrics'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_capacity_metrics")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_capacity_metrics.migrations"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Metrics & Monitoring Extension App."""
    command = "nautobot-server generate_nautobot_capacity_metrics_test_data"
    if database:
